...
import { calculateSajuAnalysis, type SajuInputObject } from "../saju";
...
export class ChatService {
  ...
  private async handleGatheringInfo(
    ...
    try {
      ...
      const recentMessages =
        await messageRepository.findRecentGatheringMessages(session.id, 20);
      const conversationHistory =
        messageRepository.formatConversationHistory(recentMessages);
      ...
      if (extractedInfo.birth_date)
        updateData.birth_date = extractedInfo.birth_date;
      if (extractedInfo.birth_time)
        updateData.birth_time = extractedInfo.birth_time;
      ...
      if (missingInfo.hasAll) {
        ...
        // 비동기로 분석 시작
        this.startBackgroundAnalysis(session.id, updatedSession).catch(
          console.error
        );
        ...
        // 추가 정보 필요
        const collectedInfo =
          infoExtractorService.formatCollectedInfo(updatedSession);
  ...
  private async handleQuestionAnswering(
    ...
    try {
      ...
      const conversationHistory =
        messageRepository.formatConversationHistory(messages);
    ...
    try {
      ...
      if (
        ...
        const input: SajuInputObject = {
          ...
          birth_time: userInfo.birth_time.substring(0, 5),
          is_lunar: false, // 추후 사용자 입력 받기
        };

        try {
          sajuAnalysisData = await calculateSajuAnalysis(input);
          // 사주 분석 데이터를 DB에 저장
          await sessionRepository.saveSajuAnalysis(
            sessionId,
            sajuAnalysisData as unknown as Record<string, unknown>
          );
          console.log("Saju analysis data calculated and saved");
        } catch (error) {
          console.error("Failed to calculate saju data:", error);
        }
      }

      // 2. AI 기반 사주 해석 수행
      const analysisResult = await sajuAnalyzerService.performInitialAnalysis(
        userInfo
      );
      ...
      // 3. 분석 결과를 세션에 저장하고 상태를 READY로 변경
